...
$norden="<b> ".$nea['codigo']."</b>";

$proyecto=get_proj($nea['destino']);
$destino=utf8_decode(" ".$proyecto['descripcion']);
...
$tf=10;
$pdf->addText($tx+6,$ty+428,$tf,"<b>Dependencia del Solicitante:</b> direccion de obras");//.$nea['dependencia']);
$pdf->addText($tx+6,$ty+410,$tf,"<b>Solicito Entregar a :</b> Ingeniero Residente / Almacen");//.$nea['pecosa']['entregarto']);
...
$pdf->addText($tx+684,$ty+400,$tf+2,"<b>NEA Nro:</b>".$norden);
...
$pdf->addText(540,73,12,"<b>Nro(".$rowsdif.") </b>".$son." Inclusive ");
...
for ($xi=0; $xi<=$rowsdif-1; $xi+=1){
	$items=$xi+1;
	$clasf=$nea['detalle'][$xi]['especifica'];
	$cant=$nea['detalle'][$xi]['cantidad'];
	$und=$nea['detalle'][$xi]['umedida'];
	$detalle=$nea['detalle'][$xi]['descripcion'];
	$punit=$nea['detalle'][$xi]['precio'];
	$ptotal=$nea['detalle'][$xi]['total'];
	$punit=number_format($punit, 2, ".", ",");
	$total=$total+$ptotal;
	$ptotal=number_format($ptotal, 2, ".", ",");
	$datadet[] = array('items'=>$items,'cant'=>$cant,'und'=>$und,'detalle'=>($detalle),'spacio'=>'','clasf'=>$clasf,'cant2'=>$cant,'punit'=>$punit,'ptotal'=>$ptotal);
}
