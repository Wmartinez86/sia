...
$ncot=" <b>".$cot['codigo']."</b>";
$referencia=" <b>".substr(utf8_decode($cot['referencia']),0,90)."</b>";
//Datos de Tabla
...
$yi=0;
	...
	while ($rowsdif>$inde and ($yi < 34))
	{
		if($rowsdif>=$inde){
				...
				if($Cot[$inde]['cantidad']==0){
					$cant="";		
					$und="";
					$detalle="<b>".utf8_decode($Cot[$inde]['descripcion'])."</b>";
						$tm=strlen($Cot[$inde]['descripcion']);
						if($tm>=70){
							$yi=$yi+(int)($tm/70);										
						}					
				}else{
					$cant=utf8_decode("<b>".$Cot[$inde]['cantidad']."</b>");		
					$und=utf8_decode("<b>".$Cot[$inde]['umedida']."</b>");
					$detalle="<b>".utf8_decode($Cot[$inde]['descripcion'])."</b>";
						$tm=strlen($Cot[$inde]['descripcion']);
						if($tm>=70){
							$yi=$yi+(int)($tm/70);										
	...
	$pdf->addText($clix,$cliy-70,$cliz,utf8_decode("<b><b>Sirva(n)se cotizarnos PRECIOS NETOS incluídos los impuestos de los siguientes ITEMS que se detallan a continuación: </b></b>"));
	...
	$pdf->line(500,655,500,165);//V1
		...
		//Cuerpo
		if ($inde>$rowsdif-1){
 			$datadet[$yi+1] = array('cant'=>"",'und'=>"",'detalle'=>"",'marca'=>"",'punit'=>"<b>TOTAL</b>",
			'ptotal'=>"");
		}
			$newdata = $datadet;
			$pdf->ezSetDy(-200);
