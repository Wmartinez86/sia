	...
	while ($rowsdif>$inde and ($yi < 23))
	{
		if($rowsdif>=$inde){
				...
				if($cot['detalle'][$inde]['cantidad']==0){ 
					...
					$detalle=$cot['detalle'][$inde]['descripcion'];
						$items=$items+1;
						$tm=strlen($cot['detalle'][$inde]['descripcion']);
						if($tm>=60){
							$yi=$yi+(int)($tm/60);										
...
//		$pdf->addText($clix+120,$cliy-20,10,"<b>Pagina </b>.".$xi." de ".($count+1));
	...
	$ty=109;
	$tf=9;
	$pdf->addText($tx+6,$ty+355,$tf,"<b>Fecha: </b><b>".$cuadro['fecha']."</b>");
	$pdf->addText($tx+6,$ty+340,$tf,utf8_decode("<b>Cotización Nro: </b><b>".$cot['codigo']."</b>"));
	$pdf->addText($tx+6,$ty+325,$tf,"<b>Referencia: </b><b>".substr(utf8_decode($cot['referencia']),0,40)."</b>");
	$pdf->addText($tx+6,$ty+310,$tf,utf8_decode("<b>Cotización elaborada el: </b><b>".$cot['fecha']."</b>"));
	
	$pdf->addText($tx+256,$ty+358,$tf,utf8_decode("<b>Nombre o Razón Social:</b>"));
	...
	$pdf->addText(240,$dy,$df,utf8_decode("<b>Descripción</b>"));
...
//	$dpf=418;
	...
	$dpy=432;
	$pdf->addText($dpx,$dpy,$df,utf8_decode($cot['prov1']['plazo']));
	$pdf->addText($dpx+138,$dpy,$df,utf8_decode($cot['prov2']['plazo']));
	$pdf->addText($dpx+276,$dpy,$df,utf8_decode($cot['prov3']['plazo']));
	//Datos del Proveedor 	
...
$cadenasa = <<<EOD
En la $entidad, siendo el día $fecha se reunieron $firmara para la apertura de sobres, evaluacion de propuestas y otorgamiento de la buena Pro, del estudio de Mercado segun la cotizacion nro. $cotizacion. teniendo como referencia el Requerimiento $ref . 

Acto seguido se procede a la apertura de propuestas de las empresas a quienes se les hizo la invitación correspondiente, y quienes remitieron en sobres cerrados:

1.- $prov1[1] con Número de RUC $prov1[2]
2.- $prov1[4] con Número de RUC $prov1[5]
3.- $prov1[7] con Número de RUC $prov1[8]
 
Seguidamente se realiza la verificación de la documentación presentada, obteniendo el cuadro comparativo adjunto a la presente acta.
...
De lo expuesto. Se resuelve otorgar la Buena Pro al Postor $prov1[10],  con RUC : $prov1[11] domiciliado en $prov1[12], cuyo monto total propuesto asciende a la cantidad de $TOTAL. Para la contratación de los bienes/servicios detallados en el cuadro comparativo antes mencionado, monto que incluye IGV  y demas gastos que puedan incidir sobre el costo del bien  a contratar, y un plazo de ejecución de $plazo contados a partir de la generacion de La Orden de Compra y/o Servicio.	

Teniendo como Justificación:
