...
$destino=substr($destino,0,80);
	...
	'oc'=>array('justification'=>'center','width'=>50)),
...
while ($rowsdif>$inde and ($yi < 23))
{
	if($rowsdif>=$inde){
		...
		$detalle=utf8_decode($pecosa['detalle'][$inde]['producto']['descripcion']);
		$tm=strlen($detalle);
			if($tm>=90){
				$yi=$yi+(int)($tm/90);
			}
		...
		if($pecosa['detalle'][$inde]['producto']['orden']['codigo']==null){$isnea=true;$oc="".$pecosa['codigo'];
		}else{$oc="".$pecosa['detalle'][$inde]['producto']['orden']['codigo'];	}		
		$items=$items+1;
	}
	$inde=$inde+1;
...
//Titulo Numeracion
		...
		$pdf->line($tx+560,$lix,$tx+560,$lix2);//V1
		$pdf->line($tx+614,$lix,$tx+614,$lix2);//V1
		$pdf->line($tx+669,$lix,$tx+669,$lix2);//V1
		$pdf->line($tx+751,$lix,$tx+751,$lix2);//V1
		...
		$pdf->addText(522,$dy,$df,"<b>Codigo</b>");
		$pdf->addText(592,$dy,$df,"<b>Cant</b>");
		$pdf->addText(638,$dy,$df,"<b>Unitario</b>");
		$pdf->addText(715,$dy,$df,"<b>Total</b>");
		//orden de Comrpa
		if($isnea){
			$pdf->addText(782,$dy,$df,"<b>NEA</b>");
		}
		else{
			$pdf->addText(782,$dy,$df,"<b>O/C</b>");
		}
		...
		$pdf->addText(565-$t_inc,73,12,$inclusives);
